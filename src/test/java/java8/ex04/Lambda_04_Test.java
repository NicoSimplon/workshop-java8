...
public class Lambda_04_Test {
	...
	public void test_filter_map_forEach_with_vars() throws Exception {
		...
		// 50
		FuncCollection<Person> filterByAge = personFuncCollection.filter(t -> t.getAge() > 50);
		...
		FuncCollection<Account> mapToAccount = filterByAge.map(t -> {
			Account a = new Account();
			a.setOwner((Person) t);
			a.setBalance(1000);
			return a;
		 ...
		 */
		personFuncCollection .filter(t -> t.getAge() > 50)
			.map(t -> {
				Account a = new Account();
				a.setOwner((Person) t);
				a.setBalance(1000);
